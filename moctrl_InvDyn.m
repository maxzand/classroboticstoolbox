function [t,q,qd,qdd,xe] = moctrl_InvDyn(dh,xdes,q0,qd0,T,Kp,Kd,g0,mL,mm,Il,Im,kr,Fv,Fs)
%
% Motion control in the operational space using inverse dynamics control. 
% 
% Inputs
%
...
% ignored
%
% xdes (nx1) symbolic function is the desired end effector position as f(time)
%
...
syms time;
xddes = diff(xdes); xdddes = diff(xddes);
...
for i = 1:N
    ...
    y = pinv(Ja)*(subs(xdddes,time,t(end)) + ...
        Kd*(subs(xddes,time,t(end))-Ja*qd(:,end)) + ...
        Kp*(subs(xdes,time,t(end))-xe(:,end)) - Jadqd );
    y = double(y);
